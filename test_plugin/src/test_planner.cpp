...
bool test_planner::TestPlanner::setPlan(const std::vector< geometry_msgs::PoseStamped > &plan) {
    ...
    while (tferr) {
	    ...
	    tf_->lookupTransform("odom", "base_footprint", ros::Time(0), stfBaseLinkWrtOdom_);
    ...
    ROS_INFO("odom to base_footprint tf is good");
...
void test_planner::TestPlanner::compute_stf_base_wrt_map() {
    ...
    stfBaseLinkWrtOdom_.child_frame_id_ = "base_footprint";
